...
class LeggedRobot(BaseTask):
    ...
    def _reward_z_vel_mismatch_exp(self):
        """
    ...
    def _reward_tracking_joint_pos(self):
        # actions_scaled = self.actions_c * self.cfg.control.action_scale
        # pos_error = actions_scaled + self.default_dof_pos - self.dof_pos
        # return torch.exp(-2.0 * torch.norm(pos_error, dim=1)) - - 0.2 * torch.norm(pos_error, dim=1).clamp(0, 0.5)
    ...
    def _reward_feet_clearance(self):
        ...
        # rew_pos = (torch.norm(feet_z, dim=-1) - 0.2).clip(min=0.)
