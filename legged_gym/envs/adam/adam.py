...
from .adam_config import AdamCfg
from legged_gym.utils.terrain import Terrain


class AdamRobot(BaseTask):
    def __init__(self, cfg: AdamCfg, sim_params, physics_engine, sim_device, headless):
    ...
    def _compute_torques(self, actions):
        ...
        actions_scaled[:, [1, 2, 7, 8]] *= self.cfg.control.hip_scale_reduction  # scale down hip flexion range

        # actions_scaled[:, 12:] *= 0.0 # scale down torso and arm actions
    ...
    def _reward_stand_still(self):
        ...
        return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos)[:, :15], dim=1) * (torch.norm(self.commands[:, :2], dim=1) < 0.1)
    ...
    def _reward_feet_clearance(self):
        ...
        rew_pos = (torch.norm(feet_z, dim=-1) - 0.05).clip(min=0., max=0.1)
    ...
    def _reward_arm_pose(self):
        return torch.sum(torch.abs(self.actions[:,15:]), dim=1)
    ...
    def _reward_waist_pose(self):
        return torch.sum(torch.abs(self.actions[:,12:15]), dim=1)